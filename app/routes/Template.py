...
def get_all_templates(db: Session = Depends(get_db),currentUser=Depends(get_current_user)):
    templates = db.query(models.ChecklistTemplate).all()
    return templates

@router.get("/{template_id}", response_model=schemas.ChecklistTemplateOut)
def get_template(template_id: int, db: Session = Depends(get_db),currentUser=Depends(get_current_user)):
...
def delete_template(template_id: int, db: Session = Depends(get_db),currentUser=Depends(get_current_user)):
    template = db.query(models.ChecklistTemplate).filter(models.ChecklistTemplate.id == template_id).first()
    if not template:
        raise HTTPException(status_code=404, detail="Template not found")
    db.delete(template)
